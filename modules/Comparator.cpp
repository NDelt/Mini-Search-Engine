...
// a의 등장 빈도가 더 높으면 false가 반환되는데, std::sort()는 정렬 기준 함수가 false(또는 음수)를 반환하면 첫 번째 매개변수(a)가 두 번째 매개변수보다 앞서도록 정렬한다.
// 등장 빈도가 동일할 경우 인덱스 값으로 비교를 수행하며, 이 때는 a의 인덱스 값이 작을 때 false가 반환되어 a가 앞쪽으로 정렬된다.
