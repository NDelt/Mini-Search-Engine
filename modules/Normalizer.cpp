...
std::string Normalizer::normalize(std::string& query) {
    ...
    for (int queryIdx = 0; queryIdx < queryLength; ++queryIdx) {
        char c = query.at((unsigned long)queryIdx);
        
        // 대문자를 소문자로 변환
        if (c >= 65 && c <= 90) {
            c += 32;
            std::string temp(1, c);
            query.replace((unsigned long)queryIdx, 1, temp);
            continue;
        }
        ...
        switch (c) {
            ...
            case '>':
                ...
                query.erase((unsigned long)removeBegin, (unsigned long)(removeIdx - removeBegin + 1));
            ...
            case '?':
            case '#':
                query.erase((unsigned long)queryIdx, 1); // queryIdx 인덱스부터 1개의 원소를 제거
                query.insert((unsigned long)queryIdx, " "); // 제거한 위치에 공백 삽입
                queryIdx -= 1;
                break;
            default:
                break;
